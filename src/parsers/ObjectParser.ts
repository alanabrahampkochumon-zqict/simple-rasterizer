...
export function parseObject(file: any): MeshObject {
    ...
    reader.onloadend = (e) => {
        ...
        for (const line of lines) {
            if (line.startsWith("vn")) {
                ...
                if (vertices.length < 4) return // Invalid vertex
                ...
                for (let i = 2; i < indices.length; ++i) {
                    ...
                    meshObject.indices.push(new IVec3(indices[0][0], indices[i-1][0], indices[i][0]))
                }
            }
        }
    }
